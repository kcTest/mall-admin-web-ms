  ...
  <div class="detail-container">
    ...
    <el-card shadow="never" style="margin-top: 15px">
      ...
      <div style="margin-top: 20px">
        ...
        <span class="font-small marker-label">基本信息</span>
      </div>
      ...
      <div style="margin-top: 20px">
        <svg-icon icon-class="marker" style="color: #606266"></svg-icon>
        <span class="font-small marker-label">收货人信息</span>
      </div>
      <div class="table-layout">
        <el-row>
      ...
      <div style="margin-top: 20px">
        <svg-icon icon-class="marker" style="color: #606266"></svg-icon>
        <span class="font-small marker-label">商品信息</span>
      </div>
      <el-table ref="orderItemTable" :data="order.orderItemList" style="width: 100%;margin-top: 20px" border
                :header-cell-style="{'text-align':'center'}" :cell-style="{'text-align':'center'}">
        <el-table-column label="商品图片" width="120px">
          <template v-slot="scope"><img :src="scope.row.productPic" style="height: 80px" alt=""></template>
        </el-table-column>
        ...
        <el-table-column label="属性" width="120px">
          <template v-slot="{row}">{{ row.productAttr | formatProductAttr }}</template>
        </el-table-column>
        <el-table-column label="数量" width="120px">
          <template v-slot="{row}">{{ row.productQuantity }}</template>
        </el-table-column>
        <el-table-column label="小计" width="120px">
          <template v-slot="{row}">￥{{ row.productPrice * row.productQuantity }}</template>
        </el-table-column>
      </el-table>
      <div style="float: right;margin: 20px">合计：<span class="color-danger">￥{{ order.totalAmount }}</span></div>
      <div style="margin-top: 60px">
        <svg-icon icon-class="marker" style="color: #606266"></svg-icon>
        <span class="font-small marker-label">费用信息</span>
      </div>
      <div class="table-layout">
        <el-row>
        ...
        <el-row>
          ...
          <el-col :span="6" class="table-cell">
            <span color="danger">￥{{ order.totalAmount + order.freightAmount }}</span>
          </el-col>
          <el-col :span="6" class="table-cell">
            <span color="danger">￥{{ order.payAmount + order.freightAmount - order.discountAmount }}</span>
          </el-col>
        </el-row>
      </div>
      <div style="margin-top: 60px">
        <svg-icon icon-class="marker" style="color: #606266"></svg-icon>
        <span class="font-small marker-label">操作信息</span>
      </div>
      <el-table ref="orderHistoryTable" :data="order.historyList" style="width: 100%;margin-top: 20px" border
                :header-cell-style="{'text-align':'center'}" :cell-style="{'text-align':'center'}">
        <el-table-column label="操作者" width="120px">
          <template v-slot="scope">{{ scope.row.operateMan }}</template>
        </el-table-column>
        ...
        <el-table-column label="订单状态" width="120px">
          <template v-slot="{row}">{{ row.orderStatus | formatStatus }}</template>
        </el-table-column>
        <el-table-column label="付款状态" width="120px">
          <template v-slot="{row}">{{ row.orderStatus | formatPayStatus }}</template>
        </el-table-column>
        <el-table-column label="发货状态" width="120px">
          <template v-slot="{row}">{{ row.orderStatus | formatDeliveryStatus }}</template>
        </el-table-column>
        <el-table-column label="备注">
          <template v-slot="{row}">{{ row.note }}</template>
        </el-table-column>
      </el-table>
    </el-card>
      ...
      <el-form :model="receiverInfo" ref="receiverInfoForm" label-width="150px">
        ...
        <el-form-item label="所在区域：">
          <v-distpicker :province="receiverInfo.receiverProvince" :city="receiverInfo.receiverCity"
                        :area="receiverInfo.receiverRegion" @selected="onSelectRegion">
          </v-distpicker>
        </el-form-item>
    ...
    <el-dialog title="修改费用信息" :visible.sync="moneyDialogVisible" width="40%">
      <div class="table-layout">
        ...
        <el-row>
          ...
          <el-col :span="6" class="table-cell"><span
              class="color-danger">￥{{ order.totalAmount + moneyInfo.freightAmount }}</span>
          </el-col>
          <el-col :span="6" class="table-cell">
...
export default {
  components: {LogisticsDialog, VDistpicker},
  data: function () {
    return {
      id: null,
  ...
  methods: {
    ...
    handleUpdateReceiverInfo: function () {
    ...
    handleUpdateMoneyInfo: function () {
    ...
    handleCloseOrder: function () {
      this.$confirm('是否要关闭订单', '提示', {
        confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning'
      }).then(() => {
        let params = new URLSearchParams();
        params.append('ids', [this.closeInfo.id]);
        params.append('note', this.closeInfo.note);
        closeOrder(this.moneyInfo).then(() => {
          this.closeDialogVisible = false;
          this.$message({message: '关闭订单成功', type: 'success', duration: 1000});
          getOrderDetail(this.id).then(response => {
            this.order = response.data;
          });
        })
    ...
    handleMarkOrder: function () {
      this.$confirm('是否要备注订单', '提示', {
        ...
        updateOrderNote(params).then(() => {
          ...
          this.$message({message: '备注订单成功', type: 'success', duration: 1000});
          getOrderDetail(this.id).then(response => {
            this.order = response.data;
          });
        })
    ...
    showUpdateReceiverDialog: function () {
    ...
    showUpdateMoneyDialog: function () {
    ...
    showCloseOrderDialog: function () {
      this.closeDialogVisible = true;
      this.closeInfo.note = null;
      this.closeInfo.id = this.id;
    },
    showMarkOrderDialog: function () {
  ...
  filters: {
    ...
    formatOrderType: function (val) {
      return val === 1 ? '秒杀订单' : '正常订单';
    },
    formatNull: function (val) {
      return val === undefined || val === null || val === '' ? '暂无' : val;
    },
    formatLongText: function (val) {
      switch (true) {
        case val === undefined || val === null || val === '':
          return '暂无';
        case val.length > 8:
    ...
    formatProductAttr: function (val) {
      if (val == null) {
        return '';
      } else {
    ...
    formatDeliveryStatus: function (val) {
      return val === 0 || val === 1 ? '未发货' : '已发货';
    }
  }
}
</script>
