<template>
  <div class="app-container">
    ...
    <div class="table-container">
      <el-table :cell-style="{'text-align':'center'}" :header-cell-style="{'text-align':'center'}" ref="productTable"
        ...
        <el-table-column label="商品图片" width="120">
          <template v-slot="scope"><img style="height: 80px" :src="scope.row.pic" alt=""></template>
        </el-table-column>
...
export default {
  ...
  methods: {
    getProductSkuSp: function (row, index) {
    ...
    getProductCateList: function () {
      fetchListWithChildren().then(response => {
        ...
        for (let i = 0; i < list.length; i++) {
          ...
          if (currentChildren.children != null && currentChildren.children.length > 0) {
            for (let j = 0; j < currentChildren.children.length; j++) {
              children.push({label: currentChildren.children[j].name, value: currentChildren.children[j].id});
            }
          }
    ...
    handleShowSkuEditDialog: function (index, row) {
    ...
    handleSearchEditSku: function () {
      fetchSkuStockList(this.editSkuInfo.productId, {keyword: this.editSkuInfo.keyword}).then(response => {
        this.editSkuInfo.stockList = response.data;
      });
    },
    handleEditSkuConfirm: function () {
    ...
    handleBatchOperate: function () {
      if (this.operateType == null) {
        this.$message({
          message: '请选择操作类型', type: 'warning', duration: 1000
        });
        return;
      }
      if (this.multiSelection == null || this.multiSelection.length < 1) {
        this.$message({
          message: '请选择要操作的商品', type: 'warning', duration: 1000
        });
        return;
      }
    ...
    handlePublishStatusChange: function (index, row) {
      let ids = [];
      ids.push(row.id);
      this.updatePublishStatus(row.publishStatus, ids);
    },
    handleNewStatusChange: function (index, row) {
      let ids = [];
      ids.push(row.id);
    ...
    updateNewStatus: function (newStatus, ids) {
      let params = new URLSearchParams();
      params.append('ids', ids);
    ...
    updateRecommendStatus: function (recommendStatus, ids) {
      let params = new URLSearchParams();
      params.append('ids', ids);
    ...
    updateDeleteStatus: function (deleteStatus, ids) {
      let params = new URLSearchParams();
      params.append('ids', ids);
