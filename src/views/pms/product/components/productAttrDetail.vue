...
export default {
  ...
  methods: {
    ...
    getEditAttrOptions: function (id) {
      ...
      for (let i = 0; i < this.value.productAttributeValueList.length.length; i++) {
        ...
        if (attrValue.productAttributeId === id) {
          ...
          for (let j = 0; j < strAttr.length; j++) {
            options.push(strAttr[j]);
          }
          break;
        }
      }
    ...
    //获取选中的属性值
    getEditAttrValues: function (index) {
      let values = new Set();
      if (index === 0) {
    ...
    handleProductAttrChange: function (value) {
      this.getProductAttrList(0, value);
      this.getProductAttrList(1, value);
    },
    getInputListArr: function (inputList) {
      return inputList.split(',');
    },
    handleAddProductAttrValue: function (idx) {
      ...
      if (options.indexOf(this.addProductAttrValue) !== -1) {
        this.$message({message: '属性值不能重复', type: 'warning', duration: 1000});
        return;
      }
      this.selectProductAttr[idx].options.push(this.addProductAttrValue);
      this.addProductAttrValue = null;
    },
    handleRemoveProductAttrValue: function (idx, index) {
    ...
    handleSyncProductSkuPrice: function () {
      this.$confirm('将同步第一个sku的价格到sku，是否继续', '提示', {
        confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning'
      }).then(() => {
    ...
    handleSyncProductSkuStock: function () {
      this.$confirm('将同步第一个sku的库存到sku，是否继续', '提示', {
        confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning'
      }).then(() => {
        if (this.value.skuStockList !== null && this.value.skuStockList.length > 0) {
          let tempSkuList = [];
    ...
    refreshProductSkuList: function () {
      this.value.skuStockList = [];
    ...
    refreshProductAttrPics: function () {
    ...
    //获取商品属性相关图片
    getProductSkuPic: function (name) {
      for (let i = 0; i < this.value.skuStockList.length; i++) {
    ...
    //合并商品属性
    mergeProductAttrValue: function () {
    ...
    handleRemoveProductSku: function (index) {
    ...
    getParamInputList: function (inputList) {
      return inputList.split(',');
    },
    handlePrev: function () {
      this.$emit('prevStep');
