    ...
    <el-form :model="productCate" :rules="rules" ref="productCateForm" label-width="150px">
      ...
      <el-form-item label="上级分类：">
        <el-select v-model="productCate.parentId" placeholder="请选择分类">
          <el-option v-for="item in selectProductCateList" :key="item.id" :label="item.name"
      ...
      <el-form-item v-for="(filterProductAttr,index) in filterProductAttrList" :lable="index | showLabelFilter"
...
import {updateProductCate, createProductCate, fetchList, getProductCate} from "@/api/productCate";
...
export default {
  ...
  methods: {
    getProductAttrCateList: function () {
    ...
    removeFilterAttr: function (filterProductAttr) {
      if (this.filterProductAttrList.length === 1) {
        this.$message({message: '至少要保留一个', type: 'warning', duration: 1000});
        return;
      }
      const index = this.filterProductAttrList.indexOf(filterProductAttr);
      if (index !== -1) {
        this.filterProductAttrList.splice(index, 1);
      }
    },
    handleAddFilterAttr: function () {
      if (this.filterProductAttrList.length === 3) {
        this.$message({message: '最多添加三个', type: 'warning', duration: 1000});
        return;
      }
      this.filterProductAttrList.push({
        value: null,
    ...
    getProductAttributeIdList: function () {
      //获取指定属性分类下的属性
      let productAttributeIdList = [];
      for (let i = 0; i < this.filterProductAttrList.length; i++) {
        let filterProductAttr = this.filterProductAttrList[i];
        if (filterProductAttr.value !== null && filterProductAttr.value.length === 2) {
          //filterProductAttr.value=[attrCateId,attrId]
          productAttributeIdList.push(filterProductAttr.value[1]);
        }
      }
      return productAttributeIdList;
    },
    getSelectProductCateList: function () {
