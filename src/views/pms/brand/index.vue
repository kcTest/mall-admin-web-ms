<template>
  <div class="app-container">
    ...
    <div class="table-container">
      <el-table :cell-style="{'text-align':'center'}" :header-cell-style="{'text-align':'center'}" ref="brandTable"
        ...
        <el-table-column label="品牌制造商" width="100">
          <template v-slot="scope">
            <el-switch @change="handleFactoryStatusChange(scope.$index,scope.row)" :active-value="1"
                       inactive-value="0" v-model="scope.row.factoryStatus"></el-switch>
          </template>
        </el-table-column>
        <el-table-column label="是否显示" width="100">
          <template v-slot="scope">
            <el-switch @change="handleShowStatusChange(scope.$index,scope.row)" :active-value="1"
                       inactive-value="0" v-model="scope.row.showStatus"></el-switch>
          </template>
        </el-table-column>
        <el-table-column label="相关" width="220">
          <template v-slot="scope">
            ...
            <span style="margin-left: 4px">评价：</span>
...
<script>
import {fetchList, updateFactoryStatus, updateShowStatus, deleteBrand} from '@/api/brand'
...
export default {
  ...
  methods: {
    ...
    handleFactoryStatusChange: function (index, row) {
    ...
    getProductList: function (index, row) {
      console.log(index, row);
      this.$message({message: 'unimplemented', type: 'warning', duration: 1000});
    },
    getProductCommentList: function (index, row) {
      console.log(index, row);
      this.$message({message: 'unimplemented', type: 'warning', duration: 1000});
    },
    ...
    handleBatchOperate: function () {
      ...
      if (this.operateType === 'showBrand') {
        ...
        this.$message({message: '请选择操作类型', type: 'warning', duration: 1000});
        return;
      }
      let ids = [];
