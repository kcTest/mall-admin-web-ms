      ...
      <div style="margin-top: 15px">
        <el-form :inline="true" :model="listQuery" size="small" label-width="140px">
          <el-form-item label="使用状态：">
            <el-select class="input-width" v-model="listQuery.useStatus" clearable placeholder="全部">
    ...
    <div class="table-container">
      <el-table :cell-style="{'text-align':'center'}" :header-cell-style="{'text-align':'center'}"
        ...
        <el-table-column label="当前状态" width="140">
          <template v-slot="scope">{{ scope.row.useStatus | formatHistoryCouponUseType }}</template>
        </el-table-column>
        <el-table-column label="使用时间" width="160">
          <template v-slot="scope">{{ scope.row.useTime | formatTime }}</template>
        </el-table-column>
...
export default {
  ...
  filters: {
    ...
    formatGetType: function (val) {
      return val === 1 ? '主动获取' : '后台赠送';
    },
    ...
    formatHistoryCouponUseType: function (val) {
      switch (val) {
        case 0:
          return '未使用';
        case 1:
