<template>
  <el-card class="form-container" shadow="never">
    <el-form :model="coupon" ref="couponForm" :rules="rules" label-width="150px" size="small">
      ...
      <el-form-item label="可使用商品：">
        <el-radio-group v-model="coupon.useType" size="small">
      ...
      <el-form-item v-show="coupon.useType===1">
        <el-cascader clearable placeholder="请选择分类名称" v-model="selectProductCate"
        ...
        <el-table ref="productCateRelationTable" :data="coupon.productCategoryRelationList"
                  style="width: 100%;margin-top: 20px" border :cell-style="{'text-align':'center'}"
                  :header-cell-style="{'text-align':'center'}">
          <el-table-column label="分类名称">
            <template v-slot="{row}">{{ row.parentCategoryName }}>{{ row.productCategoryName }}</template>
          </el-table-column>
          <el-table-column label="操作" width="100">
            <template v-slot="scope">
              <el-button size="mini" type="text" @click="handleDeleteProductCateRelation(scope.$index)">删除
              </el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <el-form-item v-show="coupon.useType===2">
        ...
        <el-table ref="productRelationTable" :data="coupon.productRelationList"
          ...
          <el-table-column label="货号">
            <template v-slot="{row}">{{ row.productSn }}</template>
          </el-table-column>
          <el-table-column label="操作" width="100">
            <template v-slot="scope">
              <el-button size="mini" type="text" @click="handleDeleteProductRelation(scope.$index)">删除
              </el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <el-form-item>
        <el-button size="primary" @click="onSubmit('couponForm')">提交</el-button>
...
const defaultCoupon = {
  type: 0, name: null, platform: 0, amount: null, perLimit: 1,
  minPoint: null, startTime: null, endTime: null,
  useType: 0, note: null, publishCount: null,
...
export default {
  ...
  methods: {
    ...
    handleAddProductCategoryRelation: function () {
      if (this.selectProductCate === null || this.selectProductCate.length === 0) {
        this.$message({message: '请选择商品分类', type: 'warning', duration: 1000});
        return;
      }
      this.coupon.productCategoryRelationList.push(this.getProductCateByIds(this.selectProductCate));
      this.selectProductCate = [];
    },
    getProductCateByIds: function (ids) {
      let name, parentName;
      for (let i = 0; i < this.productCateOptions.length; i++) {
        let parentCate = this.productCateOptions[i];
        if (parentCate.value === ids[0]) {
          parentName = parentCate.label;
          for (let j = 0; j < parentCate.children.length; j++) {
            let children = parentCate.children[j];
            if (children.value === ids[1]) {
              name = children.label;
            }
          }
        }
    ...
    handleDeleteProductCateRelation: function (index) {
    ...
    handleAddProductRelation: function () {
      if (this.selectProduct === null) {
        this.$message({message: '请选择商品', type: 'warning', duration: 1000});
        return;
      }
      this.coupon.productRelationList.push(this.getProductById(this.selectProduct));
      this.selectProduct = null;
    },
    handleDeleteProductRelation: function (index) {
